...
function calculateSunTimes(latitude: number, longitude: number, dayOfYear: number): { sunrise: SunTime; sunset: SunTime } {
...
function calculateAsr(dhuhr: SunTime, latitude: number, dayOfYear: number): SunTime {
 ...
 */
export function getApproximatePrayerTimes(cityName: string, latitude: number, longitude: number): PrayerTimings {
  const times = calculateLocalPrayerTimes(latitude, longitude);
  
  // Store in localStorage for future offline use
  const cacheKey = `local_prayer_times_${latitude.toFixed(3)}_${longitude.toFixed(3)}_${new Date().toDateString()}`;
