...
import { getWeatherContextualQuote, getPrayerContextualQuote, getTimeBasedQuote, IslamicQuote, getAlternativeContextualQuote } from "../data/islamicQuotes";
...
const PrayerWeatherInsights: React.FC<PrayerWeatherInsightsProps> = ({
  ...
  }, [current.weather, current.main.temp, current.main.humidity, nextPrayer, cityName, refreshKey]);
          ...
          <div className="quote-content">
            ...
            <div className="quote-source">
              ...
              <div className="context-info">
                <small>📍 {cityName} • 🌤️ {current.weather[0].description} • 🌡️ {Math.round(current.main.temp)}°C</small>
              </div>
            </div>
          </div>
        </div>
            ...
            {nextPrayer ? (
              <>
                ...
                <div className="time-remaining">
                  in {Math.floor(nextPrayer.timeUntil / 60)}h {nextPrayer.timeUntil % 60}m
                </div>
              </>
            ) : (
      ...
      <div className="daily-quote interactive-card">
        ...
        <div className="quote-content">
          ...
          <div className="quote-source">
            ...
            <div className="source-details">{dailyQuote.sourceDetails}</div>
            <div className="context-info">
              <small>📍 {cityName} • 🌤️ {current.weather[0].description} • 🌡️ {Math.round(current.main.temp)}°C {nextPrayer ? `• 🕌 Next: ${nextPrayer.name}` : ''}</small>
            </div>
          </div>
        </div>
      </div>
