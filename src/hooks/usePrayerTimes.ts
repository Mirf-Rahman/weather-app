...
export function usePrayerTimes(
  ...
  const getCacheKey = useCallback(
    (lat: number, lon: number, date: string) => {
      return `prayer_times_${lat.toFixed(3)}_${lon.toFixed(
        3
      )}_${date}_${method}_${school}`;
    },
    [method, school]
  );
  ...
  const loadPrayerTimes = useCallback(
    async (lat: number, lon: number, useCache: boolean = true) => {
      if (!lat || !lon) return;

      console.log("🔄 loadPrayerTimes called with:", {
        lat,
        lon,
        useCache,
        isIOS: isIOSSafari(),
      ...
      try {
          ...
          if (cached) {
            try {
              ...
              if (
                cachedData &&
                cacheTime &&
                Date.now() - parseInt(cacheTime) < 6 * 60 * 60 * 1000
              ) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            console.log(`🔄 Fetching prayer times (attempt ${attempt}/3):`, {
              lat,
              lon,
              method,
              school,
            ...
            console.log("✅ Prayer times fetch successful in hook!", {
            ...
            console.warn(
              `❌ Prayer times fetch attempt ${attempt} failed:`,
              error
            );
        ...
        if (!response && lastError) {
          console.error(
            "🚨 All prayer times attempts failed, throwing error:",
            lastError
          );
          throw lastError;
        }
        ...
        if (!response) {
          throw new Error(
            "Failed to fetch prayer times after multiple attempts"
          );
        }
        ...
        console.log("✅ State updated successfully with prayer times!", {
          isIOS: isIOSSafari(),
        });
      } catch (err) {
        const errorMessage =
        ...
        try {
          ...
          const localTimes = getApproximatePrayerTimes(
            "your location",
            lat,
            lon
          );
