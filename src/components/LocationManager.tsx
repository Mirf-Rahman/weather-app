...
export const LocationManager: React.FC<LocationManagerProps> = ({ 
  ...
  const saveRecentSearch = useCallback((location: string, country?: string) => {
    const recent = getRecentSearches();
    ...
    localStorage.setItem('weatherApp_recentSearches', JSON.stringify(trimmed));
  ...
  const saveFavoriteLocations = (locations: LocationData[]) => {
    localStorage.setItem('weatherApp_favoriteLocations', JSON.stringify(locations));
    setFavoriteLocations(locations);
  };

  const addToFavorites = (locationName: string) => {
    const existing = favoriteLocations.find(fav => 
  ...
  const removeFromFavorites = (locationName: string) => {
    const updated = favoriteLocations.filter(loc => 
                  ...
                  <div className="location-list">
                    {recentLocations.map((location, index) => (
                      <div key={`${location.name}-${index}`} className="location-item">
                          ...
                          <div className="location-info">
                            ...
                            <div className="location-meta">
                              <span className="search-count">
                                {location.searchCount} search{location.searchCount !== 1 ? 'es' : ''}
                              </span>
                        ...
                        <button
                          className={`favorite-btn ${isLocationFavorited(location.name) ? 'favorited' : ''}`}
                          onClick={() => 
                            isLocationFavorited(location.name)
                              ? removeFromFavorites(location.name)
                              : addToFavorites(location.name)
                          }
                          title={isLocationFavorited(location.name) ? 'Remove from favorites' : 'Add to favorites'}
                        >
                          {isLocationFavorited(location.name) ? '⭐' : '☆'}
                        </button>
                      </div>
            ...
            <div className="favorite-locations">
              {favoriteLocations.length === 0 ? (
                <div className="empty-state">
                  ...
                  <small>Star locations from your recent searches to add them here</small>
                </div>
              ) : (
                <>
                  <div className="location-list">
                    {favoriteLocations.map((location, index) => (
                      <div key={`fav-${location.name}-${index}`} className="location-item favorite">
                        <button
                          className="location-btn"
                          onClick={() => handleLocationClick(location.name)}
                        >
        ...
        <div className="location-manager-footer">
          {currentLocation && (
            <div className="current-location-info">
              ...
              <button
                className={`add-current-btn ${isLocationFavorited(currentLocation) ? 'favorited' : ''}`}
                onClick={() => 
                  isLocationFavorited(currentLocation)
                    ? removeFromFavorites(currentLocation)
                    : addToFavorites(currentLocation)
                }
                title={isLocationFavorited(currentLocation) ? 'Remove from favorites' : 'Add to favorites'}
              >
                {isLocationFavorited(currentLocation) ? '⭐' : '☆'}
              </button>
            </div>
